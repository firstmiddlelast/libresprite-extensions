...
// Example usage
const labColor1 = [50, 20, 10]; // Example Lab color 1
const labColor2 = [50, 20, 10]; // Example Lab color 2
const difference = calculateDeltaE(labColor1, labColor2);
//console.log("labdiff="+difference);
...
// Example usage
rgbColor = labToRgb([50, 20, 10]);
//console.log("labToRgb="+rgbColor);
...
// Example usage
var rgbColor = [128, 128, 128]; // Example RGB color
var labColor = rgbToLab(rgbColor);
//console.log("rgbToLab="+labColor);
    ...
    for (let x = 0; x < width; x += blockSize) {
        ...
        for (let blockY = 0; blockY < blockSize && y + blockY < height; blockY++) {
            for (let blockX = 0; blockX < blockSize && x + blockX < width; blockX++) {
                ...
                const [L, a, b] = rgbToLab ([color.rgbaR (pixel), color.rgbaG (pixel), color.rgbaB (pixel)]);
                totalL += L;
                totala += a;
                totalb += b;
        ...
        for (let blockY = 0; blockY < blockSize && y + blockY < height; blockY++) {
            for (let blockX = 0; blockX < blockSize && x + blockX < width; blockX++) {
                if (COLOR_MODE === "RGB") {
                    ...
                    const [pixelR, pixelG, pixelB] = labToRgb ([avgL, avga, avgb]);
