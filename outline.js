...
function fill_outline (x, y, width, height) {
    ...
    while (queue.length > 0 && fillCounter > 0) {
        ...
        for (let direction of directions) {
            ...
            if (DEBUG) log ("child="+child.x+","+child.y);
            if (child.x >= 0 && child.x < width && child.y >= 0 && child.y < height) {
                if (DEBUG) log ("testing current="+current.x+","+current.y+" child="+child.x+","+child.y);
                if (color.rgbaA (image.getPixel (current.x, current.y)) === TRANSPARENT_ALPHA && (color.rgbaA (image.getPixel (child.x, child.y)) !== TRANSPARENT_ALPHA) && OUTLINE [[child.x, child.y]] === undefined) {
                        if (DEBUG) log ("putting "+current.x+','+current.y);
    ...
    if (DEBUG) log ("End of fill, queue length=" + queue.length);
}
...
if (DEBUG) log ("height="+image.height+",width="+image.width);
for (var y = 0; y < image.height; y ++) {
  for (var x = 0; x < image.width; x ++) {
      if (DEBUG) log (x+","+y+"="+((color.rgbaA(image.getPixel(x, y))===TRANSPARENT_ALPHA)?"transparent":"opaque"));
